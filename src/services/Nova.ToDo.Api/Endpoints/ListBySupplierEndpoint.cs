...
/// <summary>
/// List by supplier: <c>POST /api/v1/todos/list/by-supplier</c>
/// Required: supplier_code (leading zeros are significant — treat as string).
/// Optional: done_ind, due_date range, include_frozen.
/// Note: SupplierCode uses the same accountmast table as clients,
/// distinguished by AccountType = 'SU'.
/// </summary>
public static class ListBySupplierEndpoint
{
    public static void Map(RouteGroupBuilder group)
    {
        group.MapPost("/todos/list/by-supplier", HandleAsync)
             .RequireAuthorization()
             .WithName("ToDoListBySupplier");
    }

    private static async Task<IResult> HandleAsync(
        ListBySupplierRequest request,
        ...
        CancellationToken     ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.SupplierCode))
            domainErrors["supplier_code"] = ["supplier_code is required."];
        ...
        var conditions = new List<string> { "t.SupplierCode = @SupplierCode" };
        ...
        string sql = $"""
            ...
            LEFT JOIN {acctMast} s  ON s.accountcode = t.SupplierCode
                                    AND s.AccountType = 'SU'
            WHERE {where}
    ...
    private sealed record ListBySupplierRequest : RequestContext
    {
