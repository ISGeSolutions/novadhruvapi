...
/// <summary>
/// Tests for POST /api/v1/branches.
/// Authentication and tenant validation are exercised without a real database.
/// Happy-path tests (200 with branch list / 404 for empty tenant) require a live
/// database and are not included here.
/// </summary>
public class BranchesEndpointTests
{
    ...
    [Fact]
    public async Task Given_NoJwt_When_BranchesIsCalled_Then_Returns401()
    {
        // Arrange
        ...
        // Act
        var response = await client.PostAsJsonAsync("/api/v1/branches", ValidBody());
    ...
    [Fact]
    public async Task Given_MissingTenantId_When_BranchesIsCalled_Then_Returns400()
    {
        // Arrange
        var client = TestHost.Create().CreateClient();
        ...
        // Act
        var response = await client.PostAsJsonAsync("/api/v1/branches", body);
    ...
    [Fact]
    public async Task Given_MissingUserId_When_BranchesIsCalled_Then_Returns400()
    {
        // Arrange
        var client = TestHost.Create().CreateClient();
        var token  = JwtFactory.CreateToken(
    ...
    [Fact]
    public async Task Given_TenantIdMismatchBetweenJwtAndBody_When_BranchesIsCalled_Then_Returns403()
    {
        // Arrange
        var client = TestHost.Create().CreateClient();
